...
class PAN_conv1x1down(FPN):
    ...
    def __init__(
        ...
        super(PAN_conv1x1down, self).__init__(
            in_channels,
            out_channels,
            num_outs,
            start_level,
        ...
        # Add 1x1 conv layers for bottom-up path
        self.bottom_convs = nn.ModuleList()
        for i in range(len(in_channels) - 1):
            bottom_conv = nn.Conv2d(
                out_channels, out_channels, kernel_size=1, stride=2, padding=0
            )
    ...
    def forward(self, inputs):
        ...
        for i in range(0, used_backbone_levels - 1):
            inter_outs[i + 1] += self.bottom_convs[i](inter_outs[i])
